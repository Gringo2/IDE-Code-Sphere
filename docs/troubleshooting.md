...
rm -rf ~/snap/codesphere/common/.cache
fc-cache -r
```
...
- blurry screen with HiDPI on wayland run:
  ```bash
  flatpak override --user --nosocket=wayland com.CodeSphere.codesphere
  ```
...
- SDKs
  see [this](https://github.com/flathub/com.CodeSphere.codesphere?tab=readme-ov-file#sdks)

- If you have any other problems with the flatpak package try to look on the [FAQ](https://github.com/flathub/com.CodeSphere.codesphere?tab=readme-ov-file#faq) maybe the solution is already there or open an [issue](https://github.com/flathub/com.CodeSphere.codesphere/issues).
...
- try the command `codesphere --verbose`
...
- try `codesphere --ozone-platform=x11`
