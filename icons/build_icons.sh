...
if ! declare -F load_linux_png &>/dev/null; then
  load_linux_png() {
    wget "https://raw.githubusercontent.com/CodeSphere/icons/main/icons/linux/circle1/${COLOR}/paulo22s.png" -O "$1"
  }
fi

if ! declare -F load_windows_ico &>/dev/null; then
  load_windows_ico() {
    wget "https://raw.githubusercontent.com/CodeSphere/icons/main/icons/win32/nobg/${COLOR}/paulo22s.ico" -O "$1"
  }
fi
...
build_darwin_main() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/darwin/code.icns" ]]; then
    ...
    if [[ "$1" == "no-template" ]]; then
      rsvg-convert -w 1024 -h 1024 "icons/${QUALITY}/codesphere_cnl.svg" -o "code_1024.png"
    else
      rsvg-convert -w 655 -h 655 "icons/${QUALITY}/codesphere_cnl.svg" -o "code_logo.png"
...
build_darwin_types() { # {{{
  if [[ "$1" == "no-border" ]]; then
    rsvg-convert -w 128 -h 128 "icons/${QUALITY}/codesphere_cnl.svg" -o "code_logo.png"
  else
    rsvg-convert -w 128 -h 128 "icons/${QUALITY}/codesphere_cnl_w80_b8.svg" -o "code_logo.png"
  fi
...
build_media() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg" ]]; then
    ...
    cp "icons/${QUALITY}/codesphere_clt.svg" "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg"
...
build_windows_type() { # {{{
  ...
  if [[ ! -f "${FILE_PATH}" ]]; then
    ...
    rsvg-convert -w "${LOGO_SIZE}" -h "${LOGO_SIZE}" "icons/${QUALITY}/codesphere_cnl.svg" -o "code_logo.png"
...
build_windows_types() { # {{{
  ...
  rsvg-convert -b "#F5F6F7" -w 64 -h 64 "icons/${QUALITY}/codesphere_cnl.svg" -o "code_logo.png"
